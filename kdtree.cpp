...
namespace kdtree {
	...
	void _build_recursively(std::span<Vector2f> points, bool split_x) {
		const size_t size = points.size();
		if (size <= 1) return;
		std::sort(points.begin(), points.end(), split_x ? _compare_x : _compare_y);
		const size_t left_size = size / 2;
		_build_recursively(points.first(left_size), !split_x);
		const size_t right_size = size - left_size - 1;
		_build_recursively(points.last(right_size), !split_x);
	}

	void build(std::span<Vector2f> points) {
		_build_recursively(points, true);
	}
}