...
{
	int key_modifier_state = RmlSFML::get_key_modifier_state();

	switch (ev.type)
	{
	case sf::Event::MouseMoved:
		return context->ProcessMouseMove(ev.mouseMove.x, ev.mouseMove.y, key_modifier_state);
	case sf::Event::MouseButtonPressed:
		return context->ProcessMouseButtonDown(ev.mouseButton.button, key_modifier_state);
	case sf::Event::MouseButtonReleased:
		return context->ProcessMouseButtonUp(ev.mouseButton.button, key_modifier_state);
	case sf::Event::MouseWheelMoved:
		return context->ProcessMouseWheel(float(-ev.mouseWheel.delta), key_modifier_state);
	...
	case sf::Event::TextEntered: {
		...
		if (ev.text.unicode >= 32 || c == Rml::Character('\n'))
			return context->ProcessTextInput(c);
		else
			return true;
	}
	case sf::Event::KeyPressed:
		return context->ProcessKeyDown(RmlSFML::sfml_key_to_rml_key(ev.key.code), key_modifier_state);
	case sf::Event::KeyReleased:
		return context->ProcessKeyUp(RmlSFML::sfml_key_to_rml_key(ev.key.code), key_modifier_state);
	default:
		return true;
	}
}

Rml::Input::KeyIdentifier RmlSFML::sfml_key_to_rml_key(sf::Keyboard::Key key)
{
	switch (key)
	{
...
Rml::Input::KeyModifier RmlSFML::get_key_modifier_state()
{
	...
	return (Rml::Input::KeyModifier)modifiers;
}
